...
class WeeklySidebar extends StatelessWidget {
  const WeeklySidebar({
    super.key,
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    'Fill "${eventTypes[currentStep].name}" time\n'
                    'Recommended: 3 hours\n'
                    'Current: ${_calculateHours()} hours',
                    style: const TextStyle(
                      fontSize: 16,
                    ),
                  ),
  ...
  Widget _buildLegend() {
    const legendItems = [
      ['Health', Color(0xFF8A9A86)],
      ['Community',  Color(0xFF007AFF)],
      ['Friends', Color(0xFFB5A1E2)],
      ['Travel', Color(0xFFFFD166)],
      ['Spiritual', Color(0xFF5C5292)],
      ['Admin', Color(0xFF8E8E93)],
      ['Cultural', Color(0xFFF3A78D)],
      ['Marketing', Color(0xFFFF7A00)],
      ['Family', Color(0xFF2C7A7B)],
      ['Financial', Color(0xFF2E7D32)],
      ['Personal', Color(0xFF2C7A7B)],
      ['On the Tools', Color(0xFF4A6572)],
    ];
  ...
  String _calculateHours() {
    ...
    for (final event in events) {
      if (event.eventType == eventTypes[currentStep].name) {
        final start =
...
class ScheduleTabHomePage extends StatefulWidget {
  const ScheduleTabHomePage({super.key});

  @override
  State<ScheduleTabHomePage> createState() =>
      _ScheduleTabHomePageState();
}

class _ScheduleTabHomePageState
    extends State<ScheduleTabHomePage> {
  ...
  builder: (context, constraints) {
    ...
    return Row(
      ...
      children: [
        ...
        if (isHorizontal)
          WeeklySidebar(
            currentStep: _stepTracker,
            events: events,
            onAddSchedule: () async {
              final newEvent = await _showAddSchedule(
                context,
                _stepTracker,
                events,
              );
